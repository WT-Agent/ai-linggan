  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">创意类型</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">受众画像</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">创新思考与脑暴发散方法论指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 创意灵感与脑暴点子专家');
...
const metricsList = [
  {
    "key": "creativeOriginality",
    "label": "创意切入点新颖度"
  },
  {
    "key": "crossIndustryInspiration",
    "label": "跨界联想启发度"
  },
  {
    "key": "feasibilityIndex",
    "label": "商业落地可行度"
  },
  {
    "key": "brainstormingRichness",
    "label": "发散思维丰富度"
  },
  {
    "key": "userResonancePotential",
    "label": "受众共鸣潜力值"
  }
];
const guideRules = [
  {
    "title": "SCAMPER替代法则",
    "advice": "通过替换、合并、改变、挪用、消除、反转 6 个维度强行进行跨界联想",
    "avoid": "切忌在发想初期就进行批判或自我否定"
  },
  {
    "title": "MVP最小验证",
    "advice": "将大创意拆解为成本最低的小测试，快速投放市场获取真实用户反馈",
    "avoid": "避免沉迷于无法落地的高大上虚幻概念"
  },
  {
    "title": "情绪钩子卡位",
    "advice": "好的创意必然切中人类恐惧、希望、自豪或好奇等核心情绪痛点",
    "avoid": "切勿做自嗨型无痛无痒的自吹自擂创意"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[LINGGAN_SCORES\][\s\S]*?\[\/LINGGAN_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[LINGGAN_SCORES\](.*?)\[\/LINGGAN_SCORES\]/);
...
const loadHistory = () => {
  try {
    const raw = localStorage.getItem('linggan_history_records');
...
const saveHistory = () => {
  localStorage.setItem('linggan_history_records', JSON.stringify(historyList.value));
};
